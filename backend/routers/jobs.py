...
async def list_jobs(
    ...
    for job, match in unique_results:
        ...
        jobs.append(JobResponse(
            ...
            gemini_strengths=[s for s in (match.gemini_strengths or []) if s],
            gemini_gaps=[s for s in (match.gemini_gaps or []) if s],
            missing_domains=[s for s in (match.missing_domains or []) if s],
